...
`robots.txt` and `llms.txt` at the root are hand-maintained (not regenerated by the build). `robots.txt` explicitly allowlists AI-search crawlers (GPTBot, ChatGPT-User, OAI-SearchBot, ClaudeBot, anthropic-ai, PerplexityBot, Google-Extended, Applebot-Extended). `llms.txt` follows the [llmstxt.org](https://llmstxt.org) spec and is a markdown summary for LLMs — keep it in sync when site structure changes meaningfully.
